...
MODEL_PATH = "checkpoints/ppo_pykachu_update_450.pt"  # change to a specific checkpoint if desired
...
obs = ppo_agent.downsample_obs(obs)
agent = ppo_agent.PikachuPPO(ppo_agent.ACTION_DIMS).to(ppo_agent.DEVICE)
...
while running:
    ...
    obs_tensor = ppo_agent.obs_to_tensor(obs).unsqueeze(0)
